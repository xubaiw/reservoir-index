...
def lQueens(n, f = sys.stdout, binary = False, careful = False, info = False, zdd = circuit.Z.none, interleave = False, storeFile=None):
    encoding = "binary" if binary else "one-hot"
    ckt = circuit.Circuit(f)
...
def lqgen(n, binary = False, careful = False, info = False, zdd = circuit.Z.none, interleave = False, storeFile=None):
    ...
    try:
        ...
        print "Couldn't open file %d" % fname
        sys.exit(1)
    lQueens(n, f, binary, careful, info, zdd, interleave, storeFile)
