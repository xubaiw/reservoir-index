...
def split_impur (impur : List α -> Float) (x_labels : List (Float × α)) (thr : Float) : Float :=
...
def best_split (impur : List α -> Float) (x : List Float) (labels : List α) : Float × Float :=
    let x_l := List.zip x labels
    let x_l := sort (fun (a, _) (b, _) => a ≤ b) x_l
    let rec loop remaining_x_l best_thr best_impur :=
        ...
        | (x1, l1) :: (x2, l2) :: t =>
            let new_thr := (x1 + x2) / 2
            let new_impur := split_impur impur x_l new_thr
